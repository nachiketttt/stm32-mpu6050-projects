...
volatile uint32_t g_channel_1_state=8000;
volatile uint32_t g_channel_2_state=4000;

acceleratometer_data_t accel_data1;
const acceleratometer_data_t error_offset={
		.x=250,
		.y=100,
		.z=-150,
};
...
int main(void)
{
  ...
  KalmanFilter kf_var;
  kalman_filter_init(&kf_var);
  HAL_Delay(100);


  if(mpu6050_init(&hi2c1,0x68)!=MPU6050_OK)
	  Error_Handler();


  if(mpu6050_config_low_pass_filter(&hi2c1,DLPF_CFG_21HZ) != MPU6050_OK)
	  Error_Handler();
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if(mpu6050_read_accelerometer_data(&hi2c1, mpu6050_i2c_addr, &accel_data1)!=MPU6050_OK)
		  Error_Handler();

	  accel_data1=mpu6050_accelerometer_calibration(&error_offset, &accel_data1);

	  roll_angle=atan2(accel_data1.y,accel_data1.z)*(180.0/M_PI);

	  kalman_roll_angle=kalman_filter_get_angle(&kf_var, roll_angle, dt);
	  ...
	  uint32_t pwm_pulse=map(kalman_roll_angle, ANGLE_POS_MIN, ANGLE_POS_MAX, \
